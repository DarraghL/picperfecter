...
const Home: NextPage = () => {
  ...
  return (
    <div
      className="flex max-w-6xl mx-auto flex-col items-center justify-center py-2 min-h-screen w-full"
      style={{ backgroundColor: '#1E1E1E' }} // Apply background color to the entire page
    >
      <Head>
        ...
        <style>{`
          body, html {
            ...
            padding: 0;
            height: 100%;
            background-color: #1E1E1E; // Ensure background color is applied to the body and html
          }
        `}</style>
      </Head>
      ...
      <main className="flex flex-1 w-full flex-col items-center justify-center text-center px-4 mt-4 sm:mb-0 mb-8">
        <h1 className="mx-auto max-w-4xl font-display text-4xl font-bold tracking-normal text-slate-200 sm:text-6xl mb-5">
          Restore any face photo
        </h1>
        {status === 'authenticated' && data && (
          <p className="text-slate-500">
            You have <span className="font-semibold">{data.remainingGenerations} generations</span> left today. Your generation
            {Number(data.remainingGenerations) > 1 ? 's' : ''} will renew in{' '}
            <span className="font-semibold">{data.hours} hours and {data.minutes} minutes.</span>
          </p>
        )}
        <div className="flex justify-between items-center w-full flex-col mt-4">
          ...
          {status === 'loading' ? (
            <div className="max-w-[670px] h-[250px] flex justify-center items-center">
              <Rings height="100" width="100" color="black" radius="6" visible />
            </div>
            ...
            <div className="h-[250px] flex flex-col items-center space-y-6 max-w-[670px] -mt-8">
              <div className="max-w-xl text-gray-600">
                Sign in below with Google to create a free account and restore your photos today. You will be able to restore 2 photos per day for free.
              </div>
              <button
          ...
          {error && (
            <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-xl mt-8 max-w-[575px]" role="alert">
              <div className="bg-red-500 text-white font-bold rounded-t px-4 py-2">Please try again in 24 hours</div>
